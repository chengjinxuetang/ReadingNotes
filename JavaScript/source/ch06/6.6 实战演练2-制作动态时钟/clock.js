...
var sDaye=sWeek[myDate.getDay()]; //根据得到的数字星期,利用数组转换成汉字星期
...
//输入日期和星期
document.getElementById("date").innerHTML=(sYear + "年" + sMonth + "月" + sDate + "日" + "星期" + sDaye + "<br>");
    h=formatTwoDigits(h)  //格式化小时,如果不足两位在前面补0
    m=formatTwoDigits(m)  //格式化分钟,如果不足两位在前面补0
    s=formatTwoDigits(s)  //格式化秒钟,如果不足两位在前面补0
    //显示时间
...
//将数转换为图像，注意，在本文件的相同目录下已有0~9的图像文件，文件名为0.png,l,png,以此类堆()
